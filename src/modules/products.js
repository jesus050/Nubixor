     ...
     LEFT JOIN LATERAL (
       ...
       FROM (
         ...
         WHERE link.company_id = p.tenant_id AND link.entity_type = 'PRODUCT'
           AND link.entity_id = p.id AND link.purpose = 'PRIMARY_IMAGE'
           AND media.deleted_at IS NULL
         UNION ALL
         SELECT legacy.public_url, 1 priority, legacy.created_at
         FROM product_images legacy
         WHERE legacy.tenant_id = p.tenant_id AND legacy.product_id = p.id
       ) image_source
